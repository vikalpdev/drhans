...
<section class="relative bg-gradient-to-r from-navy-600 to-navy-700 overflow-hidden">
    ...
    <div class="relative mx-auto max-w-7xl px-6 py-8 flex flex-col lg:flex-row items-center justify-between gap-5 text-white">
        ...
        <div class="grid grid-cols-2 gap-3 w-full sm:flex sm:flex-wrap sm:w-auto sm:justify-center">
            <a href="{{ $siteSettings->phoneUrl() }}" class="group inline-flex items-center justify-center gap-2 bg-white hover:bg-mint-50 text-navy-700 font-heading font-semibold px-5 py-2.5 rounded-full text-sm shadow-md hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200">
                <x-app-icon name="phone" class="w-4 h-4 transition-transform duration-200 group-hover:scale-110" /> Call Now
            </a>
            <a href="{{ $siteSettings->whatsappUrl() }}" target="_blank" class="group inline-flex items-center justify-center gap-2 border-2 border-white/50 text-white hover:bg-white/10 hover:border-white font-heading font-semibold px-5 py-2.5 rounded-full text-sm hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200">
