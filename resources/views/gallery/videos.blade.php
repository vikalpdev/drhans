<x-layouts.app title="Video Gallery" description="Watch patient testimonials and stories of recovery from Dr Hans' Centre for ENT patients across our hearing, vertigo and ENT care programs.">
    ...
    >
        @if ($videos->count())
            <div class="flex flex-nowrap sm:flex-wrap gap-2.5 mb-8 bg-mint-50 border border-navy-100 rounded-2xl p-2 w-full sm:w-fit overflow-x-auto sm:overflow-visible [&::-webkit-scrollbar]:hidden" style="scrollbar-width: none;">
                <button
                    @click="cat = 'all'"
                ...
                @foreach ($categories as $category)
                    <button
                        ...
                        class="shrink-0 px-4 py-2 rounded-full text-sm font-heading font-medium transition-colors duration-200"
